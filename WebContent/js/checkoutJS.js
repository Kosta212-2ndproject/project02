...
function isChecked() {
    if ($('input[name="agree"]:checked').val() == null) {
        alert("약관을 동의해주세요.");
    } else if ($('input[class="mr-2 pay-method"]:checked').val() == null) {
        alert("결제 방식을 선택해주세요.")
    } else {
        productBuy();
    }
...
function addressUpdate() {
    if (!$('input:radio[id=addressInsert]').is(':checked')) {
        alert("체크 해주세요.")
    } else {
        //해당하는 회원의 휴대폰번호가 있으면 주소 저장
        $.ajax({
            url: "../userAddressUpdate", // 서버요청주소
            type: "get",  // 전송방식(get.post, put, delete)
            dataType: "text",   // 서버가 응답해주는 데이터의 타입(html, text, xml, json 중의 한 개 )
            data: {
                userId: $("#userId").val(),
                userZipCode: $("#post-code").val(),
                userAddr: $("#address").val() + ' ' + $("#extraAddress").val(),
                userAddrDetail: ' ' + $("#detail-address").val(),
        ...
        if (rsp.success) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    orderInsert(rsp.merchant_uid, rsp.paid_amount, rsp.paid_at, userId);
